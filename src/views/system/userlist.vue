...
export default {
  ...
  methods: {
    ...
    CheckCustomerAuth(aid) {
      ...
      if (
        String(this.customers.authList).split(",").indexOf(String(aid)) != -1
      ) {
        return true;
      }
    },
    ChangeCustomerAuth(aid) {
      if (this.customers.authList == "all") {
        this.customers.authList = this.menuAllList
          .filter((item) => item !== String(aid))
          .join(",");
        return;
      }
      let FFList = String(this.customers.authList).split(",");
      if (FFList.indexOf(String(aid)) != -1) {
        this.customers.authList = FFList.filter(
          (item) => item !== String(aid)
        ).join(",");
        return;
      } else {
    ...
    EditOne(RowItem) {
      ...
      if (RowItem.id == null || RowItem.id == 0) {
        ...
        this.customers.authList =
          RowItem.authList == null || RowItem.authList == ""
            ? "all"
            : RowItem.authList;
      }
    ...
    ReInitMenu() {
      ...
      let F1List = this.customersData.filter(
        (x) => String(x.id) == String(user.UserID)
      );
      if (F1List.length == 1) {
        localStorage.setItem(
          "user_authList",
          JSON.stringify(F1List[0].authList)
        );
      }
    },
    AddData(data1) {
              ...
              <div class="col-sm-4">
                <div class="text-sm-end">
                  ...
                  <b-modal
                    size="l"
                    v-model="showModal"
                    :title="customers.id == 0 ? '新增用戶' : '修改用戶'"
                    ...
                    <form @submit.prevent="handleSubmit">
                      <div class="row">
                        <div class="col-6">
                          <div class="mb-3">
                            ...
                            <input
                              ...
                              :class="{
                                'is-invalid':
                                  submitted && v$.customers.loginName.$error,
                              }"
                            />
                            <div
                              v-if="submitted && v$.customers.loginName.$error"
                              class="invalid-feedback"
                            >
                              <span
                                v-if="v$.customers.loginName.required.$message"
                                >{{
                                  v$.customers.loginName.required.$message
                                }}</span
                              >
                            </div>
                            <span
                        ...
                        <div class="col-6">
                          ...
                          <select
                            class="form-select"
                            v-model="customers.roleId"
                            :class="{
                              'is-invalid':
                                submitted && v$.customers.roleId.$error,
                            }"
                          >
                            <option
                              :value="u1.id"
                              selected
                              v-for="u1 in rolelist"
                              :key="'customers_roleid' + u1.id"
                            >
                              {{ u1.name }} ({{ u1.type }})
                            </option>
                          </select>
                          <div
                            v-if="submitted && v$.customers.roleId.$error"
                            class="invalid-feedback"
                          >
                            <span
                              v-if="v$.customers.roleId.required.$message"
                              >{{ v$.customers.roleId.required.$message }}</span
                            >
                          </div>
                        </div>
                        <div class="col-6">
                          ...
                          <div class="mb-3">
                            ...
                            <div class="btn-group-sm mb-3">
                              <a
                                ...
                                @click="AuthCheckALL(true)"
                                >全選</a
                              >
                              <a
                                href="javascript:;"
                                class="btn btn-sm btn-outline-secondary"
                                @click="AuthCheckALL(false)"
                                >不選</a
                              >
                            </div>
                            <div
                              class="form-check"
                              v-for="(m1, midx) in menuItems"
                              :key="'authList' + midx"
                            >
                              <div class="form-check form-check-inline">
                                <i v-if="m1.icon" class="bx" :class="m1.icon"
                                  >&nbsp;</i
                                >
                                <input
                                  autocomplete="off"
                                  ...
                                  type="checkbox"
                                  name="authCheck"
                                  :checked="CheckCustomerAuth(m1.id)"
                                  @click="ChangeCustomerAuth(m1.id)"
                                  v-if="!m1.subItems"
                                  :value="m1.id"
                                />
                              ...
                              <template v-if="m1.subItems">
                                ...
                                >
                                  <input
                                    ...
                                    type="checkbox"
                                    name="authCheck"
                                    :checked="CheckCustomerAuth(m2.id)"
                                    @click="ChangeCustomerAuth(m2.id)"
                                    :value="m2.id"
                                  />
                                  <label
                                    :for="'authid' + m2.id"
