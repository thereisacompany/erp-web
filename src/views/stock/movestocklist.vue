...
export default {
  ...
  validations: {
    customers: {
      ...
      anotherDepotId: {
        required: helpers.withMessage("請選擇移入倉庫", required),
      },
  ...
  methods: {
    ...
    BatchSetStatus() {
      ...
      let idsList = this.customersData
        .filter((x) => x.chk == true && x.status == 5)
        .map((y) => y.subId);
    ...
    SingleSetStatus() {
      ...
      let ConfirmAmount = "";
      if (
        this.customersItem[0].amount != null &&
        this.customersItem[0].amount != "" &&
        this.customersItem[0].amount >= 0
      ) {
        ConfirmAmount = `\n確認數量:${this.customersItem[0].amount}`;
      }
      if (
        confirm(
          `確認完成單號:${this.customers.defaultNumber}移倉單\n商品:${this.customersItem[0].name}?${ConfirmAmount}`
        ) != true
      ) {
        return;
      }
    ...
    SetInvalidStatus() {
      ...
      if (
        confirm(
          `確認作廢單號:${this.customers.defaultNumber}移倉單\n商品:${this.customersItem[0].name}?`
        ) != true
      ) {
        return;
      }
    ...
    GetData() {
      ...
      APIParameter += `&subType=${encodeURIComponent("移倉")}&keyword=${
        this.queryKeyword
                  ...
                  >
                    ...
                    <td>
                      <div class="btn-group btn-group-sm">
                        <span class="btn btn-success" v-if="SubItem.status == 4"
                          >已完成</span
                        >
                        <span class="btn btn-warning" v-if="SubItem.status == 5"
                          >移倉中</span
                        >
                        <span class="btn btn-danger" v-if="SubItem.status == 6"
                          >作廢</span
                        >
                      </div>
                    </td>
                    <td>
                      <div class="btn-group btn-group-sm">
                        <a
                          class="btn btn-info"
                          href="javascript:;"
                          @click="EditShow(SubItem, 3)"
                          >查看</a
                        >
                        <a
                          class="btn btn-secondary"
                          href="javascript:;"
                          v-if="SubItem.status == 5"
                          @click="EditShow(SubItem, 2)"
                          >編輯</a
                        >
                      </div>
                    </td>
